...
import com.qiqiim.webserver.user.service.InstancemessagedayService;
...
public class MessageProxyImpl implements MessageProxy {
	...
	public void saveOnlineMessageToDB(MessageWrapper message) {
    	try{
			Instancemessageday  userMessage = convertMessageWrapperToBean(message);
    		if(userMessage!=null){
				userMessage.setIsRead(Instancemessageday.ISREAD_ENUM.YES.getIsRead());
				instancemessagedayService.insertSelective(userMessage);
    		}
    	}catch(Exception e){
	...
	public void saveOfflineMessageToDB(MessageWrapper message) {
    	try{
			...
			Instancemessageday  userMessage = convertMessageWrapperToBean(message);
    		if(userMessage!=null){
    			userMessage.setIsRead(Instancemessageday.ISREAD_ENUM.NO.getIsRead());
            	instancemessagedayService.insertSelective(userMessage);
    		}
    	}catch(Exception e){
    		 log.error("MessageProxyImpl  user "+message.getSessionId()+" send msg to "+message.getReSessionId()+" error");
    ...
    private Instancemessageday convertMessageWrapperToBean(MessageWrapper message){
    	try{
    		//保存非机器人消息
    		if(!message.getSessionId().equals(Constants.ImserverConfig.REBOT_SESSIONID)){
				...
				Instancemessageday  userMessage = new Instancemessageday();
            	userMessage.setSendUserId(message.getSessionId());
            	userMessage.setRecUserId(message.getReSessionId());
            	userMessage.setContext(msgConten.getContent());
				if(msgConten.getContent().indexOf("img[")>=0&&msgConten.getContent().indexOf("]")>0){
					userMessage.setMsgType(Instancemessageday.MSGTYPE_ENUM.IMG.getMsgType());
				}else if(msgConten.getContent().indexOf("file(")>=0&&msgConten.getContent().indexOf(")[下载文件]")>0){
					userMessage.setMsgType(Instancemessageday.MSGTYPE_ENUM.FILE.getMsgType());
				}else{
					userMessage.setMsgType(Instancemessageday.MSGTYPE_ENUM.TXT.getMsgType());
				}
            	userMessage.setSendTime(DateUtil.nowDate());
            	userMessage.setIsRead(Instancemessageday.ISREAD_ENUM.YES.getIsRead());
