...
public class ImConnertorImpl implements ImConnertor{
	...
	@Override
	public void SyspushMessage(MessageWrapper wrapper,String content) throws RuntimeException {
		try {
			///取得接收人 给接收人写入消息
			Session responseSession = sessionManager.getSession(wrapper.getReSessionId());
			Instancemessageday userMessage = new Instancemessageday();
			userMessage.setSendUserId(wrapper.getSessionId());
			userMessage.setRecUserId(wrapper.getReSessionId());
